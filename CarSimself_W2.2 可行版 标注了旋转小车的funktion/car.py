...
class Car:
    ...
    def update(self, game_map):
        ...
        # else:
        #     position_tmp = self.Lenkeinschlagsänderung(position_tmp)
    ...
    def Lenkeinschlagsänderung(self, position):

        maxmaleangle = 10
        minmaleangle = -10  # 最大转向角
        ...
        car_radius = CAR_Radstand / math.tan(angle_rad)   # 半径
        theta = math.degrees(self.speed / car_radius) * (simulation.time_flip)
       ...
       #  position[0] += CAR_Radius * (math.cos(math.radians(self.carangle)) - math.cos(math.radians(self.carangle + theta)))
        dx = self.speed * car_radius * (math.sin(math.radians(self.carangle + theta)) - math.sin(math.radians(self.carangle)))
        dy = self.speed *  car_radius * (math.cos(math.radians(self.carangle)) - math.cos(math.radians(self.carangle + theta)))
        position[0] += dx
        position[1] += dy
