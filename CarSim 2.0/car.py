...
CAR_SIZE_X = real_to_sim(40)  # 23.75*2 * f
CAR_SIZE_Y = real_to_sim(20)
...
class Car:
    ...
    def __init__(self, position, angle, speed, speed_set, radars,  bit_volt_wert_list, distance, time):
    ...
    def rebound_action(self, game_map, point):
        ...
        angle_reflection = np.clip(angle_reflection, -180, 180)
    ...
    def update(self, game_map):
        ...
        position_tmp[0] += math.cos(math.radians(360 - self.angle)) * self.speed
        ...
        # Same For Y-Position
        position_tmp[1] += math.sin(math.radians(360 - self.angle)) * self.speed
        position_tmp[1] = max(position_tmp[1], 20*f)
        position_tmp[1] = min(position_tmp[1], WIDTH - 120*f)

        # Calculate New Center
        self.center = [int(position_tmp[0]) + CAR_SIZE_X / 2, int(position_tmp[1]) + CAR_SIZE_Y / 2]
