...
class Car:
    ...
    def check_collision(self, game_map):
        ...
        for point in self.corners:
            ...
            if game_map.get_at((int(point[0]), int(point[1]))) == BORDER_COLOR:
                status = simulation.switch_button.get_status()
                # crash the car
                if status == 0:
                    self.alive = False
                elif status == 1:
                    ...
                    for i in range(0, 360, 10):
                        ...
                        if game_map.get_at([int(x), int(y)]) == BORDER_COLOR and game_map.get_at(
                                [int(point[0] + radius * math.cos(angle + 15)),
                                 int(point[1] + radius * math.sin(angle + 15))]) == (0, 0, 0, 0):
                            point1 = [int(x), int(y)]
                            break
    ...
    def check_radar(self, degree, game_map):  # , midlength):
        ...
        while not game_map.get_at((x, y)) == BORDER_COLOR and length < 45 * f:
            length = length + 1
            x = int(self.center[0] + math.cos(math.radians(360 - (self.angle + degree))) * length)
            y = int(self.center[1] + math.sin(math.radians(360 - (self.angle + degree))) * length)
