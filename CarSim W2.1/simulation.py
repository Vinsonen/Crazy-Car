...
def run_simulation(genomes, config):
    ...
    for i, g in genomes:
        ...
        # _init__(self, position, angle, speed, speed_set, radars,  bit_volt_wert_list, distance, time)
        new_car = Car([210 * f, 250 * f], 60, 0, False, [], [], 0, 0)   # [790 * f, 825 * f]
        cars.append(new_car)
    ...
    while True:
        ...
        # Once sumtime
        counter += 1
        if counter == 100 * 60:  # Stop After 100FPS * S
            break
...
def regelung(cars):
    ...
    for i, car in enumerate(cars):
        if car.radars_enable:
            jia = 0.3 * f  # 注意：是像素加速度 不是实际的 加速度大小
            ...
            if car.angle_enable:
                ...
                if distcm[0] <= 90:
                    car.angle += 10
                    if distcm[1] <= 80:
                        if car.speed - jia > 5 * f:
                            car.speed -= jia
                    else:
                        if car.speed + jia < 6 * f:
                            car.speed += jia
                elif distcm[2] <= 90:
                    car.angle -= 10
                    if distcm[1] <= 80:
                    ...
                    if car.speed + jia < 6 * f:
                        car.speed += jia
                elif distcm[0] == distcm[2] and distcm[0] <= 80:  # 垂直碰撞
                    car.angle += 10

                car.angle = np.clip(car.angle, -180, 180)
            else:
                break
